 ...
 * 
 * Key Features:
 * - Real-time price recording from trades
 * - OHLC (Open, High, Low, Close) data generation
 * - Multiple timeframe support for charts
 * - Price aggregation and statistics
 * - Current price retrieval
 * 
 * Price Algorithm (from spec 1.4):
 ...
 * - Max ±20% change per month to prevent manipulation
 */
 ...
 * @param volume - Trade volume (shares)
 ...
 * Used when aggregating trades into time periods
 * 
...
export async function recordOHLC(data: {
  ...
  console.log(`[PriceHistoryService] Recording OHLC for company ${data.companyId}: O=${data.open}, H=${data.high}, L=${data.low}, C=${data.close}`);

  const priceRecord = await prisma.priceHistory.create({
    data: {
      companyId: data.companyId,
      price: data.close, // Use close as the main price
      volume: data.volume,
      timestamp: data.timestamp,
      // OHLC fields would need to be added to schema
      // For now, we store them in metadata
      metadata: {
        open: data.open,
        high: data.high,
        low: data.low,
 ...
 * @returns Map of companyId to price
 */
 ...
 * 
 * @returns Array of company price info
 */
 ...
 * Supports multiple timeframes with appropriate aggregation
 * 
 * @param companyId - Company ID
 * @param timeframe - Time period for data
...
export async function getPriceHistoryOHLC(
  ...
  // Group trades by time period and calculate OHLC
 ...
 * @returns Price change amount and percentage
 */
...
// ============================================================================
// MONTHLY PRICE ADJUSTMENT (From Specification 1.4)
// ============================================================================

/**
 * Calculate new stock price based on company performance
 * This implements the algorithm from specification section 1.4
 * 
 * @param companyId - Company ID
...
export async function calculateNewPrice(companyId: string): Promise<number> {
  ...
  // Get trading volume for last month
  ...
  // Check dividend consistency (paid in last 3 months)
  ...
  const dividendScore = Math.min(dividendCount / 3, 0.1); // Cap at 10%

  // Calculate performance score (weighted)
  const performanceScore =
    (revenueGrowth * 0.4) +      // 40% weight on revenue growth
    (profitMargin * 0.3) +       // 30% weight on profit margin
    (volumeScore * 0.2) +        // 20% weight on trading activity
    (dividendScore * 0.1);       // 10% weight on dividend payments

  // Calculate new price
  let newPrice = currentPrice * (1 + performanceScore);

  // Cap at ±20% per month to prevent manipulation
  ...
  console.log(`[PriceHistoryService] Price calculation: $${currentPrice} → $${newPrice} (score: ${performanceScore.toFixed(4)})`);
 ...
 * This should be run as a cron job at the beginning of each month
 */
...
function aggregatePriceData(
  ...
  // For longer timeframes, aggregate data points
  if (timeframe === PriceTimeframe.ONE_HOUR || timeframe === PriceTimeframe.ONE_DAY) {
    return rawData; // Return raw data for short timeframes
  }

  // For longer timeframes, sample data points
  const sampleRate = Math.ceil(rawData.length / 100); // Max 100 points
...
/**
 * Map Prisma price history to interface
 */
